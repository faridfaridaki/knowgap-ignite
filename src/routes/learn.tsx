...
function LearnPage() {
  ...
  const loadLesson = useCallback(() => {
    ...
    let cancelled = false;
    generate({ data: { topic: s.topic, missedConcepts: target, language: s.language } })
      .then((res) => {
        if (cancelled) return;
    ...
    return (
      <main className="min-h-screen w-full bg-background flex items-center justify-center px-6 relative">
        ...
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">{t("buildingCourse")}</p>
        </div>
      </main>
    );
  }
